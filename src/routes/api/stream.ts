...
// The upstream CDN only allows CORS from pluto.tv, so the browser can't fetch
// it directly — this route proxies the playlist and its segments, rewriting
// every URI in m3u8 files to point back through the proxy.
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        // Don't trust content-type: this CDN labels binary AES keys as
        // mpegurl. Go by the URL path and verify the body is a playlist.
